...
>
  <div
    class="w-3/4 md:w-1/2 lg:ml-10 p-6 bg-black lg:bg-transparent rounded-xl flex flex-col justify-start items-start gap-2 ml-4"
  >
    <h3 class="text-blue-800 text-lg font-semibold lg:font-bold lg:text-xl">
      ${item.title}
    </h3>
    <h1 class="text-black text-2xl lg:text-4xl font-extrabold">
      ${item.info}
    </h1>
    <h4 class="text-black lg:text-xl lg:mb-4">
      ${item.price}
    </h4>
    <button
      class="font-semibold px-3 py-2 text-xs text-white bg-blue-800 rounded-xl"
      type="button"
    >
      ${item.button}
    </button>
...
>
<div
class="w-12 h-12 border-2 bg-black rounded-lg flex items-center justify-center"
 >
 <img class="w-8 h-8" src="${item.icon}" title="productPic">
</div>
<div class="mr-auto ml-2">  
  <h3 class="font-semibold md:font-bold text-xs md:text-sm text-black mb-1">${item.title}</h3>
  <button type="button" class="border-0 text-xs md:text-sm font-semibold text-blue-800">Show All</button>
</div>
...
document.getElementById("blog_swiper").innerHTML = BolgSwiper.map((item) => (
  ...
  <h3 class="text-blue-800">${item.title}</h3>
  <h2 class="font-semibold text-sm lg:text-lg hover:text-blue-800 cursor-pointer">${item.info}</h2>
...
document.getElementById("newProducts").innerHTML = NewProducts.map(
  ...
  >
    ...
    <h3 class="text-blue-800">${item.productName}</h3>
